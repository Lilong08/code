...
def kmedoids_run(X, n_clusters, dist_func, max_iter=1000, tol=0.001, verbose=True):
    '''run algorithm return centers, members, and etc.'''
    # Get initial centers
    n_samples, n_features = X.shape
    init_ids = get_init_centers(n_clusters, n_samples)
    if verbose:
        print(
        'Initial centers are ', init_ids)
    centers = init_ids
    members, costs, tot_cost, dist_mat = get_cost(X, init_ids, dist_func)
    cc, SWAPED = 0, True
    while True:
        SWAPED = False
        for i in range(n_samples):
            if not i in centers:
                for j in range(len(centers)):
                    ...
                    centers_[j] = i
                    members_, costs_, tot_cost_, dist_mat_ = get_cost(X, centers_, dist_func)
        ...
        if cc > max_iter:
            if verbose:
                print(
                'End Searching by reaching maximum iteration', max_iter)
            break
        if not SWAPED:
            if verbose:
                print(
...
class KMedoids(object):
    ...
    def __init__(self, n_clusters, dist_func=get_distance, max_iter=5000, tol=0.0001):
    ...
    def fit(self, X, plotit=False, verbose=True):
        centers, members, costs, tot_cost, dist_mat = kmedoids_run(
...
if __name__ == '__main__':
    ...
    labels_ = trainer.fit(X, plotit = True)
